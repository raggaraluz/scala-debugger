  ...
  describe("PendingExceptionSupport") {
    describe("#processAllPendingExceptionRequests") {
      it("should process all pending exceptions") {
        ...
        val expected = Seq(
          ExceptionRequestInfo(testClassName, true, false),
          ExceptionRequestInfo(testClassName + 1, true, false),
          ExceptionRequestInfo(testClassName, false, true)
        )
    ...
    describe("#processPendingExceptionRequestsForClass") {
      it("should process pending exceptions for the specified class") {
        ...
        val actions = (expected :+ ExceptionRequestInfo(testClassName + 1, true, false))
    ...
    describe("#pendingExceptionRequests") {
      it("should return a collection of all pending exceptions") {
        val expected = Seq(
          ExceptionRequestInfo("class1", true, false),
          ExceptionRequestInfo("class1", false, true),
          ExceptionRequestInfo("class2", true, false)
        )
    ...
    describe("#pendingExceptionRequestsForClass") {
      it("should return a collection of pending exceptions") {
        ...
        val actions = (expected :+ ExceptionRequestInfo("class2", true, false))
          .map(ActionInfo.apply("", _: ExceptionRequestInfo, () => {}))
    ...
    describe("#createCatchallExceptionRequestWithId") {
      ...
      it("should add a pending exception request if exception thrown") {
        ...
        (mockPendingActionManager.addPendingActionWithId _).expects(
          TestRequestId,
          ExceptionRequestInfo(null, true, false, Nil),
    ...
    describe("#createExceptionRequestWithId") {
      ...
      it("should add a pending exception request if exception thrown") {
        ...
        (mockPendingActionManager.addPendingActionWithId _).expects(
          TestRequestId,
          ExceptionRequestInfo(testClassName, true, false, Nil),
          * // Don't care about checking action
        ).returning(TestRequestId).once()
    ...
    describe("#createCatchallExceptionRequest") {
      it("should return Success(id) if the exception was created") {
        ...
        val actual = pendingExceptionSupport.createCatchallExceptionRequest(
          true,
          false
        )
      ...
      it("should add a pending exception if exception thrown") {
        val expected = Success(TestRequestId)
    ...
    describe("#createExceptionRequest") {
      it("should throw an exception if the exception name is null") {
        intercept[IllegalArgumentException] {
          pendingExceptionSupport.createExceptionRequest(
            null,
            true,
            false
          )
      ...
      it("should return Success(id) if the exception was created") {
        ...
        val actual = pendingExceptionSupport.createExceptionRequest(
          testClassName,
          true, false
        )
      ...
      it("should return a failure if pending disabled and failed to create request") {
        ...
        pendingExceptionSupport.disablePendingSupport()
        val actual = pendingExceptionSupport.createExceptionRequest(
          testClassName, true, false, extraArguments: _*
        )
