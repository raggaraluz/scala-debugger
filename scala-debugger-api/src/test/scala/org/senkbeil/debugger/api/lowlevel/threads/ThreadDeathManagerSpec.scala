...
class ThreadDeathManagerSpec extends FunSpec with Matchers with MockFactory
  with OneInstancePerTest with org.scalamock.matchers.Matchers
{
  ...
  private val threadDeathManager = new ThreadDeathManager(mockEventRequestManager) {
  ...
  describe("ThreadDeathManager") {
    describe("#threadDeathRequestList") {
      it("should contain all thread death requests in the form of id -> request stored in the manager") {
        ...
        // above
        val threadDeathManager = new ThreadDeathManager(mockEventRequestManager) {
    ...
    describe("#createThreadDeathRequest") {
      it("should create the thread death request and return Success(id)") {
        ...
        val actual = threadDeathManager.createThreadDeathRequest()
    ...
    describe("#hasThreadDeathRequest") {
      ...
      it("should return false if it does not exist") {
        ...
        val actual = threadDeathManager.hasThreadDeathRequest(TestId)
        actual should be (expected)
      }
    }
    ...
    describe("#getThreadDeathRequest") {
      ...
      it("should return None if not found") {
        ...
        val actual = threadDeathManager.getThreadDeathRequest(TestId)
        actual should be (expected)
      }
    }

    describe("#getThreadDeathArguments") {
      it("should return Some(Seq(input args)) if found") {
        ...
        val id = threadDeathManager.createThreadDeathRequest(expected: _*).get

        val actual = threadDeathManager.getThreadDeathRequestArguments(id)
        actual should be (Some(expected))
      }
      ...
      it("should return None if not found") {
        ...
        val actual = threadDeathManager.getThreadDeathRequestArguments(TestId)
        actual should be (expected)
      }
    }
    ...
    describe("#removeThreadDeathRequest") {
      ...
      it("should return false if the thread death request was not removed") {
        ...
        val actual = threadDeathManager.removeThreadDeathRequest(TestId)
        actual should be (expected)
      }
    }
  }
