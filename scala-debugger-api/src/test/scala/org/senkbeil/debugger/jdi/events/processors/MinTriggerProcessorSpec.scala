  ...
  with OneInstancePerTest with MockFactory
{
  ...
  describe("MinTriggerProcessor") {
    describe("#process") {
      it ("should return false if the total processed has not reached the min count") {
        // Verify that the processor is false for the first N-1 processes
        for (i <- 1 to testCount - 1) {
          minTriggerProcessor.process(mock[Event]) should be (false)
        }
      }

      it("should return true if the total processed has reached the min count") {
        for (i <- 1 to testCount - 1) {
          minTriggerProcessor.process(mock[Event])
        }

        // Verify that the processor is true after the first N-1 processes
    ...
    describe("#reset") {
      it("should reset the internal counter to zero") {
        ...
        // Verify that the processor is false even after the first N-1 processes
        minTriggerProcessor.process(mock[Event]) should be (false)
      }
    }
  }
