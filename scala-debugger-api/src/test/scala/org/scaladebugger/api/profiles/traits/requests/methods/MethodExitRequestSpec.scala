...
class MethodExitRequestSpec extends ParallelMockFunSpec
{
  ...
  private val TestPipelineWithData = Pipeline.newPipeline(
    classOf[MethodExitRequest#MethodExitEventAndData]
  )

  private val successMethodExitProfile = new Object with MethodExitRequest {
  ...
  private val failMethodExitProfile = new Object with MethodExitRequest {
    override def tryGetOrCreateMethodExitRequestWithData(
      className: String,
      methodName: String,
      extraArguments: JDIArgument*
  ...
  describe("MethodExitRequest") {
    describe("#tryGetOrCreateMethodExitRequest") {
      it("should return a pipeline with the event data results filtered out") {
        ...
        var actual: MethodExitEventInfo = null
      ...
      it("should capture any exception as a failure") {
        ...
        // Data to be run through pipeline
        val data = (mock[MethodExitEventInfo], Seq(mock[JDIEventDataResult]))
    ...
    describe("#getOrCreateMethodExitRequestWithData") {
      it("should return a pipeline of events and data if successful") {
        // Data to be run through pipeline
        val expected = (mock[MethodExitEventInfo], Seq(mock[JDIEventDataResult]))

        var actual: (MethodExitEventInfo, Seq[JDIEventDataResult]) = null
        successMethodExitProfile
