...
import org.scaladebugger.api.profiles.traits.info.events.{BreakpointEventInfoProfile, EventInfoProducerProfile}
...
class PureBreakpointProfileSpec extends test.ParallelMockFunSpec with JDIMockHelpers
{
  ...
  private type EI = BreakpointEventInfoProfile
  ...
  private val pureBreakpointProfile =
    new TestPureBreakpointProfile(Some(mockRequestHelper))
    ...
    describe("for custom request helper") {
      describe("#_newRequestId") {
        it("should return a new id each time") {
          val pureBreakpointProfile = new TestPureBreakpointProfile()
