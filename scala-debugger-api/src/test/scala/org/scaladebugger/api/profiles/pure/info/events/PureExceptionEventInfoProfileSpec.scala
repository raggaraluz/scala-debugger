...
import org.scaladebugger.api.profiles.traits.info.{InfoProducerProfile, LocationInfoProfile, ObjectInfoProfile}
import org.scaladebugger.api.profiles.traits.info.events._
import org.scaladebugger.api.virtualmachines.ScalaVirtualMachine

class PureExceptionEventInfoProfileSpec extends test.ParallelMockFunSpec {
  ...
  private val pureExceptionEventInfoProfile = new PureExceptionEventInfoProfile(
  ...
  describe("PureExceptionEventInfoProfile") {
    describe("#toJavaInfo") {
    ...
    describe("#catchLocation") {
      it("should return Some new location profile if there is a catch location") {
        val expected = Some(mock[LocationInfoProfile])
      ...
      it("should return None if there is no catch location") {
        ...
        val pureExceptionEventInfoProfile = new PureExceptionEventInfoProfile(
          scalaVirtualMachine = mockScalaVirtualMachine,
          infoProducer = mockInfoProducer,
          exceptionEvent = mockExceptionEvent,
          jdiArguments = mockJdiArguments
