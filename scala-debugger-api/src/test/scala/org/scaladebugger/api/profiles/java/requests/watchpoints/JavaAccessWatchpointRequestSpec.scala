...
import org.scaladebugger.api.profiles.traits.info.events.{AccessWatchpointEventInfo, EventInfoProducer}
...
class JavaAccessWatchpointRequestSpec extends ParallelMockFunSpec with JDIMockHelpers
{
  ...
  private type EI = AccessWatchpointEventInfo
  ...
  private class TestJavaAccessWatchpointRequest(
    private val customTestRequestHelper: Option[CustomTestRequestHelper] = None
  ) extends JavaAccessWatchpointRequest {
  ...
  private val javaAccessWatchpointProfile =
    new TestJavaAccessWatchpointRequest(Some(mockRequestHelper))
  ...
  describe("JavaAccessWatchpointRequest") {
    describe("for custom request helper") {
      describe("#_newRequestId") {
        it("should return a new id each time") {
          val javaAccessWatchpointProfile = new TestJavaAccessWatchpointRequest()
          val requestHelper = javaAccessWatchpointProfile.newAccessWatchpointRequestHelper()
    ...
    describe("#tryGetOrCreateAccessWatchpointRequestWithData") {
      it("should use the request helper's request and event pipeline methods") {
        ...
        val actual = javaAccessWatchpointProfile.tryGetOrCreateAccessWatchpointRequest(
          className,
          fieldName,
    ...
    describe("#accessWatchpointRequests") {
      it("should include all active requests") {
        ...
        val javaAccessWatchpointProfile = new Object with JavaAccessWatchpointRequest {
          override protected val accessWatchpointManager = mockAccessWatchpointManager
          override protected val eventManager: EventManager = mockEventManager
          override protected val infoProducer: InfoProducer = mockInfoProducer
          override protected val scalaVirtualMachine: ScalaVirtualMachine = mockScalaVirtualMachine
        ...
        val actual = javaAccessWatchpointProfile.accessWatchpointRequests
    ...
    describe("#removeAccessWatchpointRequests") {
      it("should return empty if no requests exists") {
        ...
        val actual = javaAccessWatchpointProfile.removeAccessWatchpointRequests(
          className,
          fieldName
        )
    ...
    describe("#removeAccessWatchpointRequestWithArgs") {
      it("should return None if no requests exists") {
        ...
        val actual = javaAccessWatchpointProfile.removeAccessWatchpointRequestWithArgs(
          className,
          fieldName
        )
    ...
    describe("#removeAllAccessWatchpointRequests") {
      it("should return empty if no requests exists") {
        ...
        val actual = javaAccessWatchpointProfile.removeAllAccessWatchpointRequests()
    ...
    describe("#isAccessWatchpointRequestPending") {
      it("should return false if no requests exist") {
        ...
        val actual = javaAccessWatchpointProfile.isAccessWatchpointRequestPending(
          className,
          fieldName
        )
    ...
    describe("#isAccessWatchpointRequestWithArgsPending") {
      it("should return false if no requests exist") {
        ...
        val actual = javaAccessWatchpointProfile.isAccessWatchpointRequestWithArgsPending(
          className,
          fieldName
        )
