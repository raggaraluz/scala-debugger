...
import org.scaladebugger.api.profiles.traits.info.events.{EventInfoProducerProfile, MonitorWaitedEventInfoProfile}
...
with ParallelTestExecution with MockFactory with JDIMockHelpers
{
  ...
  private type EI = MonitorWaitedEventInfoProfile
  ...
  private class TestPureMonitorWaitedProfile(
    private val customTestRequestHelper: Option[CustomTestRequestHelper] = None
  ) extends PureMonitorWaitedProfile {
  ...
  private val pureMonitorWaitedProfile =
    new TestPureMonitorWaitedProfile(Some(mockRequestHelper))
  ...
  describe("PureMonitorWaitedProfile") {
    describe("for custom request helper") {
      describe("#_newRequestId") {
        it("should return a new id each time") {
          val pureMonitorWaitedProfile = new TestPureMonitorWaitedProfile()
          val requestHelper = pureMonitorWaitedProfile.newMonitorWaitedRequestHelper()
    ...
    describe("#tryGetOrCreateMonitorWaitedRequestWithData") {
      it("should use the request helper's request and event pipeline methods") {
        ...
        val actual = pureMonitorWaitedProfile.tryGetOrCreateMonitorWaitedRequest(
