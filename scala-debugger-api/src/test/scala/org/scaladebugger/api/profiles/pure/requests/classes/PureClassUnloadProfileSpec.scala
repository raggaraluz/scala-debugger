...
import org.scaladebugger.api.profiles.traits.info.events.{ClassUnloadEventInfoProfile, EventInfoProducerProfile}
...
with ParallelTestExecution with MockFactory with JDIMockHelpers
{
  ...
  private type EI = ClassUnloadEventInfoProfile
  ...
  private val pureClassUnloadProfile =
    new TestPureClassUnloadProfile(Some(mockRequestHelper))
    ...
    describe("for custom request helper") {
      describe("#_newRequestId") {
        it("should return a new id each time") {
          val pureClassUnloadProfile = new TestPureClassUnloadProfile()
