...
import org.scaladebugger.api.profiles.traits.info.events.{ClassPrepareEventInfoProfile, EventInfoProducerProfile}
...
with ParallelTestExecution with MockFactory with JDIMockHelpers
{
  ...
  private type EI = ClassPrepareEventInfoProfile
  ...
  private class TestPureClassPrepareProfile(
    private val customTestRequestHelper: Option[CustomTestRequestHelper] = None
  ) extends PureClassPrepareProfile {
  ...
  private val pureClassPrepareProfile =
    new TestPureClassPrepareProfile(Some(mockRequestHelper))
  ...
  describe("PureClassPrepareProfile") {
    describe("for custom request helper") {
      describe("#_newRequestId") {
        it("should return a new id each time") {
          val pureClassPrepareProfile = new TestPureClassPrepareProfile()
          val requestHelper = pureClassPrepareProfile.newClassPrepareRequestHelper()
    ...
    describe("#tryGetOrCreateClassPrepareRequestWithData") {
      it("should use the request helper's request and event pipeline methods") {
        ...
        val actual = pureClassPrepareProfile.tryGetOrCreateClassPrepareRequest(
