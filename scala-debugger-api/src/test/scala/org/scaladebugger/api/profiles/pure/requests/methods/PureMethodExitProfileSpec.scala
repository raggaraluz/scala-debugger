...
import org.scaladebugger.api.profiles.traits.info.events.{EventInfoProducerProfile, MethodExitEventInfoProfile}
...
with ParallelTestExecution with MockFactory with JDIMockHelpers
{
  ...
  private type EI = MethodExitEventInfoProfile
  ...
  private val pureMethodExitProfile =
    new TestPureMethodExitProfile(Some(mockRequestHelper))
    ...
    describe("for custom request helper") {
      describe("#_newRequestId") {
        it("should return a new id each time") {
          val pureMethodExitProfile = new TestPureMethodExitProfile()
